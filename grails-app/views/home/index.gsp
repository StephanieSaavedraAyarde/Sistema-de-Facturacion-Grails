...
<div class="card">
    ...
    <div class="card-header">
        ...
        <h3>
            <strong>
                <g:message code="Transacciones" args="['List']"/>
            </strong>
        </h3>
        ...
        <span class="float-right">
            ...
            <div class="btn-group">
                <g:form controller="home" action="index" method="GET">
            ...
            <div class="btn-group">
                ...
                <g:link controller="factura" action="index" class="btn btn-primary" style="background-color:#FFA500;  border:FFA500;">
                    <g:message code="Facturas"/>
                </g:link>
            </div>
        </span>
    </div>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                <g:each in="${homeList}" var="info">
                    <tr>
                        <td>${info?.id}</td>
                        <td>${info?.nombreServicio}</td>
                        <td>${info?.nombreCliente}</td>
                        <td>${info?.nombreEmpleado}</td>
                        <td>${info?.precio}</td>
                        <td>${info?.cantidadTrabajadores}</td>
                        <td>${info?.formaPago}</td>
                        <td>${info?.fecha}</td>

                        <td>
                            <div class="btn-group">

                                <g:link controller="home" action="edit" class="btn btn-secondary" id="${info.id}" style="background-color:#FFA500">
                                    <i>Editar</i>
                                </g:link>

                                <p>&nbsp</p>

                                <g:link controller="home" action="delete" class="btn btn-secondary delete-confirmation" id="${info.id}" style="background-color:red">
                                    <i>Eliminar</i>
                                </g:link>
